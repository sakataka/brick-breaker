...
- 日常の開発操作は `vp` を使う。`pnpm` は `packageManager` として扱い、通常の dev/build/check/test は `vp` を優先する。
- `vitest` / `oxlint` / `oxfmt` を直接追加運用しない。`vite-plus` 経由の `vp test` / `vp lint` / `vp fmt` を使う。
- ローカル実行の標準シェルは `zsh` とする。シェル依存の説明やコマンド例は `bash` 前提ではなく `zsh` 互換で記述する。

## 1. プロジェクト概要

- 構成: `Vite+ + pnpm + TypeScript`
- ゲーム実行: `Phaser`（描画/入力）
- UI: `React + Zustand`
- サウンド: `Tone/WebAudio`
...
- コミット前に最低限 `vp check` と `vp test` を通す（可能なら `vp run e2e` まで実施）。
