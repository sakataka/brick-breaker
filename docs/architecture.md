...
- `tsconfig.app.json` / `tsconfig.test.json` / `tsconfig.node.json` に責務を分け、browser runtime と Node-side config を混在させない
- `TypeScript 7` native preview は `tsgo` による advisory check として扱い、shipped quality gate へ直結させない
- `knip` など既存の TypeScript API に依存するツールは `typescript` package を継続利用する
- 新規実装では `import type`、明示的な compiler option、typed constant、local declaration を優先し、`@ts-expect-error` や `Object.keys(...) as ...` を増やさない
