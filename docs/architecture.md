...
- `src/game/content/stageBlueprintCatalog.ts`
  - `encounterId -> blueprintId` と tier 別 catalog を定義
...
- `src/game/config/stageTemplateRuntime.ts`
  - blueprint を runtime 用 stage template に展開する低レベル helper を保持
...
- stage blueprint: `src/game/content/stageBlueprints.ts` と `src/game/content/stageBlueprintCatalog.ts`
...
3. `src/game/content/stageBlueprints.ts` に盤面 blueprint を追加する
