...
- `src/game-v2/content/stageBlueprints.ts`
...
- stage 固有の brick layout / modifier / boss 初期値 / music cue は stage blueprint に集約する
...
- campaign 累計時間は `run.elapsedSec`、現在 encounter の時間は `encounter.elapsedSec` で分けて扱う
- active skill cooldown は `combat.activeSkill`、combo window は `run.comboWindowRemainingSec` で engine が管理する

### 5. Presenter

- `src/game-v2/presenter/defaultViews.ts`
- `src/game-v2/presenter/projectors.ts`
...
- projector の public export は `projectors.ts` に維持し、内部は HUD / Overlay / Shop / Render / Visual の単位で分ける
...
3. `src/game-v2/session/RuntimeController.ts` が host / audio / UI sync を調停する
4. `src/game-v2/engine/transitions.ts` と `tick.ts` が runtime state を更新する
5. `src/game-v2/presenter/projectors.ts` が HUD / Overlay / Shop / Render に投影する
6. Phaser / React / Audio がそれぞれの port で同期する

E2E 用の `window.__brickBreakerTest` は production の既定 build には含めず、`VITE_BRICK_BREAKER_TEST_BRIDGE=1` の明示時だけ `src/main.ts` から公開する。
