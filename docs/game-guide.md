...
- 開始前設定の `Stickyアイテム有効` がOFFのとき、`sticky` は出現しません
...
- 購入にはスコアコストが必要です（ラン内購入回数で価格上昇）。
- 価格は `1200 -> 1600 -> 2200 -> ...` のように1.35倍で増加します（100点単位丸め）。
- 各ステージで「無料リロール」を1回だけ使用できます（購入前のみ）。
- プレイ中は画面上段の情報バー内にショップUIを固定表示し、下段のプレイ領域とは重ならない構成です。
...
- プレイ中UIは「上段情報バー（HUD+ショップ） / 下段ゲーム枠」の2分割です。
...
- `ボス確認` プリセットは `shield=2, pierce=1, bomb=1, laser=2, sticky=1` を付与します（新アイテムスタックOFF時は `laser=1`、Sticky無効時は `sticky=0`）。
...
- 落下アイテムは「色付きタイル + 絵文字 + 短縮文字」で表示されます。
- 絵文字が見えにくい環境でも、短縮文字で種類を判別できます。
- ボールは円形、アイテムは角丸スクエアで描画されるため、動体でも見分けやすくしています。
