...
describe("gamePipeline", () => {
  ...
  test("focus activation consumes score and enables slow-time window", () => {
    const config = { ...GAME_CONFIG, width: 260, height: 180, fixedDeltaSec: 1 / 60 };
    const state = createInitialGameState(config, true, "playing");
    state.scene = "playing";
    state.score = 1200;
    state.combat.focusRequest = true;
    state.bricks = [{ id: 1, x: 30, y: 40, width: 36, height: 10, alive: true }];
    overrideSingleBall(state, {
      pos: { x: 210, y: 150 },
      vel: { x: 0, y: -80 },
    ...
    expect(state.combat.focusRemainingSec).toBeGreaterThan(0);
  });
  ...
  test("stage modifiers with spawnEnemy can trigger timed reinforcements", () => {
    ...
    expect(state.enemies.length).toBeGreaterThan(0);
    expect(state.combat.enemyWaveCooldownSec).toBeGreaterThan(0);
  });
});
