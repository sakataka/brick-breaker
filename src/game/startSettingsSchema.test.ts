...
describe("startSettingsSchema", () => {
  ...
  test("toggleEnabledItem keeps at least one item active", () => {
    ...
    expect(
      toggleEnabledItem({ enabledItems: ITEM_ORDER as ItemType[] }, ITEM_ORDER[0], false).enabledItems
        ?.length,
    ).toBe(ITEM_ORDER.length - 1);
    expect(
      toggleEnabledItem({ enabledItems: [ITEM_ORDER[0] as ItemType] }, ITEM_ORDER[0], false).enabledItems,
