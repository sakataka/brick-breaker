...
export function getThemeDefinition(themeBandId: ThemeBandId): ThemeDefinition {
  return THEME_DEFINITIONS.find((theme) => theme.id === themeBandId) ?? THEME_DEFINITIONS[0];
}

function getThemeBandByStageIndex(stageIndex: number): ThemeDefinition {
  if (stageIndex >= 8) {
    return getThemeDefinition("chapter3");
  }
  if (stageIndex >= 4) {
    return getThemeDefinition("chapter2");
  }
  return getThemeDefinition("chapter1");
}
